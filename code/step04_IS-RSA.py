...
Two analyses:
  1. After-movie: brain ISC vs. post-movie impression similarity
  2. Before-movie: brain ISC from runs 5-10 vs. pre-movie impression similarity (runs 1-6)
...
print('After-movie: significant ROIs (FDR-corrected p < 0.01):')
...
print('Before-movie: significant ROIs (FDR-corrected p < 0.01):')
...
# Labels: purple=0, yellow=1, green=3.
# avg(purple=0, green=3) = 1.5, which is NOT yellow(1), but with equal-width bins
# [0,1), [1,2), [2,3] it would still fall in the yellow bin.
# BoundaryNorm fixes this: bin edges [-0.5, 0.5, 1.5, 3.5] put 1.5 in the green bin.
# We pre-apply the norm so plot_surf_stat_map receives clean 0/1/2 indices.
_bnorm = BoundaryNorm([-0.5, 0.5, 1.5, 3.5], ncolors=3, clip=True)
...
surf_left_ov  = _apply_bnorm(surf_left_ov)
surf_right_ov = _apply_bnorm(surf_right_ov)
