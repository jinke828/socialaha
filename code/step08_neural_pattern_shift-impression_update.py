"""
step08_neural_pattern_shift-impression_update.py

Tests whether neural pattern shifts around character aha moments
(≥2/3 rater consensus) correlate with impression updates across subjects.

Pipeline:
  1. Compute impression updates: cosine dissimilarity of sentence embeddings
     between consecutive runs, per subject × scene (33 × 40).
  2. Extract neural pattern shifts in a [−5, +3] TR window around character
     aha timepoints, per ROI × subject × scene × TR (116 × 33 × 40 × 9).
  3. For each ROI × TR: Spearman correlate (Fisher z-transformed) neural
     shifts with impression updates across subjects, per scene, then average
     over the 40 scenes.
  4. Permutation null (n=1000): load pre-computed per-scene null r-values from
     results/neural_updates/null/scene/null{i}.mat (generated by scene_null.py),
     or compute them from scratch if the files are missing.
  5. Two-stage FDR: within-ROI across TPs, then within-TP across ROIs.
  6. Load step07 significant ROIs (elevated pattern shift vs. null) and apply
     double threshold: ROI must survive both step07 AND step08 correction.
  7. Save brain surface maps under results/figures/neural_update/.

  
Inputs:
  data/brain/pattern_shift/1TR_nearbytp.npy
  data/beh/embeddings/speech-embeddings_byrun_bychar.npy
  data/beh/annotations/ahaannot_all.xlsx
  socialaha-collab/socialaha-fMRI/socialaha_groupscene.csv
  [step07 significant ROIs are hardcoded — no file dependency]

Outputs:
  results/figures/neural_update/raw_TR{t}.png
  results/figures/neural_update/thresholded_step08_TR{t}.png
  results/figures/neural_update/doublethresh_TR{t}.png
  results/neural_updates/step08_shifts_perscene.npy   (cached)
  results/neural_updates/step08_rvals.npy
  results/neural_updates/null/scene/null{i}.mat        (input, from scene_null.py)
  results/neural_updates/step08_sig_rois_step08.npy
  results/neural_updates/step08_sig_rois_double.npy
"""
...
FIG_DIR        = os.path.join(RESULTS_DIR, 'figures', 'neural_update')
SCENE_NULL_DIR = os.path.join(RESULTS_DIR, 'neural_updates', 'null', 'scene')
os.makedirs(FIG_DIR, exist_ok=True)
os.makedirs(os.path.join(RESULTS_DIR, 'neural_updates'), exist_ok=True)
os.makedirs(SCENE_NULL_DIR, exist_ok=True)
...
# ---------------------------------------------------------------------------
# Character-order helpers (replicate notebook's rearrange / char_order)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Impression update: cosine dissimilarity between consecutive runs
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Annotation lookup: pre-filter pandas once per condition
# ---------------------------------------------------------------------------
...
def _shift_windows(this_run, trs_list):
    ...
    for tp in trs_list:
        ...
        windows.append(arr)
    return np.nanmean(np.array(windows), axis=0)   # (9,)
...
def compute_shifts_perscene(df_lookup, pattern_shift, flist, nroi, event_idxs):
    ...
    return np.array(results)   # (nroi, nsubj, 40, 9)
...
# ---------------------------------------------------------------------------
# Permutation null: load pre-computed or generate via scene_null approach
# ---------------------------------------------------------------------------
def load_scene_null_rvals(null_dir, n_perms=1000):
    ...
    return np.array(results)   # (n_perms, nroi, 9)
...
# ---------------------------------------------------------------------------
# Statistical testing (matching step08_test-null.ipynb exactly)
# ---------------------------------------------------------------------------
def test_significance(rvals, null_rvals, nroi_cor=100):
    ...
    Matches the procedure in step08_test-null.ipynb:
      for each TR: compute onetail_p for each cortical ROI, apply FDR BH.
    Returns:
